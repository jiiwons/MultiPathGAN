...
class MPG_solver():
    def __init__(self, loader, args):
        ...
        self.num_domains = max(args.num_domains, 4)
    ...
    def train(self):
        ...
        # If resuming training from previous checkpoint
        if self.resume_training:
            begin = self.resume_training
            self.restore_train(self.resume_training)
        ...
        for i in range(begin, self.iters):
            ...
            # Debugging: Print real_lbl values
            print('real_lbl:', real_lbl)

            # Ensure labels are within the valid range
            if real_lbl.max() >= self.num_domains:
                raise ValueError(f"Label value {real_lbl.max()} exceeds num_domains {self.num_domains}")
            ...
            print('onehot_real_lbl shape:', onehot_real_lbl.shape)
            print('onehot_tgt_lbl shape:', onehot_tgt_lbl.shape)
    ...
    def label_to_onehot(self, data_labels, data_dim):
        ...
        # Debugging: Print data_labels values
